...
# Function to create a plot with descriptive labels
create_colorful_plot <- function(model, effect, label) {
  pred <- ggpredict(model, terms = effect)
  ...
  ggplot(pred, aes(x = x, y = predicted)) +
    ...
    geom_point(data = model@frame, aes(x = .data[[effect]], y = transAge_tidymodels_LOIOCV), 
    ...
    labs(
      title = paste("Predicted Values for", effect_label),
      x = effect_label,
...
fixed_effects <- names(fixef(best_model))[-1]  # Exclude intercept
plots <- list()
labels <- LETTERS[1:length(fixed_effects)]

for (i in seq_along(fixed_effects)) {
  plots[[i]] <- create_colorful_plot(best_model, fixed_effects[i], labels[i])
}
...
combined_plot <- plots[[1]] + plots[[2]] + plots[[3]] +
  plot_layout(ncol = 3) +
  plot_annotation(
    title = "Predicted Epigenetic Age for Fixed Effects",
    theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 16))
  )

# Save the combined plot
ggsave("predicted_values_combined_plot.png", combined_plot, width = 18, height = 6, dpi = 300)
